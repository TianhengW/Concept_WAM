...
cd "${REPO_ROOT}"
export LOGDIR="slurm_logs/eval_10ep_aug110k"
export RESDIR="${REPO_ROOT}/evaluate_results/robotwin/eval_10ep_aug110k"
mkdir -p "${LOGDIR}" "${RESDIR}"

D="${REPO_ROOT}/runs/robotwin_flux2_klein_4b_actionpatch_aug_full/2026-08-23_06-34-17"
...
echo "[launcher] aug110k seed=${SEED} groups=$(echo ${GLIST} | wc -w) nodes=${SLURM_JOB_NODELIST}"
bash exploration/action_img_patch/eval_10ep_worker.sh
echo "[launcher] aug110k seed=${SEED} COMPLETE"
