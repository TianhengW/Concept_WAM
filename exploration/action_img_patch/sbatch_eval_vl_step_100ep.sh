...
OUT="${REPO_ROOT}/evaluate_results/robotwin/vl_${STEP}_100ep_fast"
LOGDIR="slurm_logs/vl_${STEP}_100ep_fast"; mkdir -p "$LOGDIR" "$OUT"
COMMON="task=robotwin_flux2_klein_4b_actionpatch_vl_full EVALUATION.dataset_stats_path=${STATS} model.flux2_src_path=/storage/yukaichengLab/mazijian/wth/flux2 model.flux2_model_path=/storage/yukaichengLab/share/model/FLUX.2-klein-base-4B/flux-2-klein-base-4b.safetensors model.ae_model_path=/storage/yukaichengLab/share/model/FLUX.2-dev/ae.safetensors model.variant=klein-base-4b model.vl_model_path=/storage/yukaichengLab/share/model/Qwen3-VL-4B-Instruct model.pack_proprio_after_text=true EVALUATION.action_horizon=16 EVALUATION.replan_steps=16 model.proprio_dim=14 EVALUATION.eval_num_episodes=100 EVALUATION.skip_get_obs_within_replan=true EVALUATION.robotwin_camera_layout=compact_288x256"
...
GLIST=""; for t in ${TASKS}; do GLIST="${GLIST} ${t}:demo_clean ${t}:demo_randomized"; done
is_done(){ local l=$1; [ -f "$l" ] && sed "s/\x1b\[[0-9;]*m//g" "$l" 2>/dev/null | grep -qE "Success rate: [0-9]+/100"; }
echo "[vl_${STEP}_100ep] 1node8gpu, skip-enabled"
j=0
for grp in ${GLIST}; do
  ...
  ( timeout -s KILL 10000 "$PY" experiments/robotwin/eval_robotwin_single.py \
...
wait
echo "=== VL ${STEP} 100ep DONE (launched=$j) ==="
