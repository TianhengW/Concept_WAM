#!/usr/bin/env bash
# action-as-patch scheme B (action-specific sigma shift, ratio 0.25), 1 node x 8.
...
EXTRA_ARGS="${1:-}"
echo "[sbatch] ap sigmashift 1node | node=${SLURM_JOB_NODELIST} | extra=${EXTRA_ARGS}"
...
/soft/slurm/bin/srun --export=ALL --kill-on-bad-exit=1 bash -c "
  ...
  export TASK=robotwin_flux2_klein_4b_actionpatch_sigmashift_sub20
