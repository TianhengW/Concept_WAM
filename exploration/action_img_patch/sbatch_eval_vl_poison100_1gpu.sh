...
OUT="${REPO_ROOT}/evaluate_results/robotwin/vl_${STEP}_100ep_retry"
LOGDIR="slurm_logs/vl_${STEP}_100ep_retry"; mkdir -p "$LOGDIR" "$OUT"
...
echo "[H100 100ep vl_${STEP}] node=$(hostname) FORCE-RUN groups=${GROUPS}"
j=0
for grp in ${GROUPS}; do
  task=${grp%%:*}; cfg=${grp##*:}; LOG="${LOGDIR}/${task}_${cfg}.log"
  gpu=0
  echo "[gpu$gpu] start $task $cfg"
  export ROBOTWIN_SKIP_SEEDS_FILE="${REPO_ROOT}/slurm_logs/deadlock_seeds/${task}_${cfg}.txt"
  ( timeout -s KILL 5000 "$PY" experiments/robotwin/eval_robotwin_single.py \
...
wait
echo "=== H100 100ep ${STEP} DONE (launched=$j) ==="
