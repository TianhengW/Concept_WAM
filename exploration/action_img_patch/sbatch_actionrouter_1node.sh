#!/usr/bin/env bash
# action-router, 1 full node x 8 GPUs.
# Hyper-parameters identical to the 1/20 AP line: effective world_size=8,
# global batch = 4 x 8 x accum 2 = 64, lr 1e-4 cosine, 10 epochs, seed 42.
...
echo "[sbatch] action-router 1node | node=${SLURM_JOB_NODELIST}"
...
/soft/slurm/bin/srun --export=ALL --kill-on-bad-exit=1 bash -c "
  ...
  GPU_PER_NODE=8 bash scripts/flux2/train_flux2_klein_imagewam.sh 8 wandb.mode=offline
"
