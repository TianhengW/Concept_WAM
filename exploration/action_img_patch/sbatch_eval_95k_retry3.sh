...
#SBATCH -x nv-h100-029,nv-h100-007,gnho006,gnho020,gnho041,gnho035
#SBATCH -J eval_95k_retry3
...
cd "${REPO_ROOT}"; mkdir -p slurm_logs/retry95k
...
CKPT="${REPO_ROOT}/runs/robotwin_flux2_klein_4b_actionpatch_full/2026-08-10_19-24-58/checkpoints/weights/step_095000.pt"
STATS="runs/robotwin_flux2_klein_4b_actionpatch_full/2026-08-10_19-24-58/dataset_stats.json"
OUT="${REPO_ROOT}/evaluate_results/robotwin/retry95k"
...
run_one() {
  ...
  timeout -s KILL 1800 python experiments/robotwin/eval_robotwin_single.py \
    ckpt="${CKPT}" gpu_id=${gpu} EVALUATION.task_name=${task} EVALUATION.task_config=${cfg} \
    EVALUATION.output_dir="${OUT}/${task}_${cfg}" ${COMMON} \
    > "slurm_logs/retry95k/${task}_${cfg}.log" 2>&1
  echo "[done] ${task} ${cfg} rc=$?"
}
run_one move_stapler_pad demo_randomized 0 &
run_one place_mouse_pad   demo_clean      1 &
run_one stamp_seal        demo_randomized 2 &
wait
echo "=== RETRY 3-GROUP RESULTS ==="
for f in slurm_logs/retry95k/*.log; do echo "--- $(basename $f) ---"; grep -E "Success rate:" "$f" | tail -1; done
echo "RETRY DONE"
