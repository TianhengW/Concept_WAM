#!/bin/bash
# 100ep dual-phase eval of aug-mixed run ckpt step_030000 (50 tasks x clean+random x 100ep).
...
cd "${REPO_ROOT}"
export LOGDIR="slurm_logs/eval_10ep_c2r30k"
export RESDIR="${REPO_ROOT}/evaluate_results/robotwin/eval_10ep_c2r30k"
mkdir -p "${LOGDIR}" "${RESDIR}"

D="${REPO_ROOT}/runs/robotwin_flux2_klein_4b_actionpatch_c2r/2026-08-27_04-12-13"
export CKPT="${D}/checkpoints/weights/step_030000.pt"
...
TASKS="put_object_cabinet shake_bottle_horizontally stack_bowls_two adjust_bottle place_burger_fries place_dual_shoes place_object_basket place_shoe rotate_qrcode stack_blocks_three"
GLIST=""
