#!/bin/bash
# 100ep dual-phase eval of aug-mixed run ckpt step_005000 (50 tasks x clean+random x 100ep).
...
cd "${REPO_ROOT}"
export LOGDIR="slurm_logs/eval_10ep_c2r005000"
export RESDIR="${REPO_ROOT}/evaluate_results/robotwin/eval_10ep_c2r005000"
mkdir -p "${LOGDIR}" "${RESDIR}"

D="${REPO_ROOT}/runs/robotwin_flux2_klein_4b_actionpatch_c2r/2026-08-27_04-12-13"
export CKPT="${D}/checkpoints/weights/step_005000.pt"
...
echo "[launcher] c2r005000 seed=${SEED} groups=$(echo ${GLIST} | wc -w) nodes=${SLURM_JOB_NODELIST}"
bash exploration/action_img_patch/eval_10ep_worker.sh
echo "[launcher] c2r005000 seed=${SEED} COMPLETE"
