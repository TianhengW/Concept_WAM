#!/usr/bin/env bash
# exploration/action_img_patch: single-stream "action-as-image-patch", 4 nodes x 8 GPUs (FULL no-op-filtered data).
...
srun --export=ALL --kill-on-bad-exit=1 bash -c '
  ...
  export TASK=robotwin_flux2_klein_4b_actionpatch_timealign_full
