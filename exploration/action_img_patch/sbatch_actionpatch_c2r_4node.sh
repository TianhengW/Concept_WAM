...
echo "[sbatch] action-as-patch C2R clean-only 4node | nodes=${SLURM_JOB_NODELIST} master=${MASTER_ADDR}:${MASTER_PORT}"

# 32 GPUs x batch 4 x accum 2 = global 256 (C2R clean-only, max_steps=30000).
srun --export=ALL --kill-on-bad-exit=1 bash -c '
  export NNODES="${SLURM_NNODES}"
  export NODE_RANK="${SLURM_NODEID}"
  export PYTHONPATH="'"${REPO_ROOT}"':${PYTHONPATH:-}"
  export TASK=robotwin_flux2_klein_4b_actionpatch_c2r
