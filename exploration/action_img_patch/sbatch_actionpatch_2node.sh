#!/usr/bin/env bash
# exploration/action_img_patch: single-stream "action-as-image-patch", 2 nodes x 8 GPUs.
# Hyper-parameters identical to sbatch_baseline_2node.sh (only the model differs).
#SBATCH -p yukaichenglab
#SBATCH -N 2
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=64
#SBATCH -x nv-h100-029,nv-h100-007,gnho006,gnho020
#SBATCH -J ap_patch_2node
...
echo "[sbatch] action-as-patch 2node | nodes=${SLURM_JOB_NODELIST} master=${MASTER_ADDR}:${MASTER_PORT}"

# 16 GPUs x batch 4 x accum 2 = global 128. Data: 1/20 subsample (stride in task yaml).
