#!/usr/bin/env bash
# accelerate launcher for Arm A. Works single- or multi-node via env:
#   NNODES, NODE_RANK, MASTER_ADDR, MASTER_PORT, GPU_PER_NODE.
# Hydra overrides are passed through as extra args.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/env_v2.sh"
...
TASK_BASENAME="${TASK_BASENAME:-robotwin_v2_armA}"
...
echo "[launch_v2] nodes=${NUM_MACHINES} rank=${MACHINE_RANK} master=${MAIN_PROCESS_IP}:${MAIN_PROCESS_PORT} gpus=${GPU_PER_NODE} zero=${ZERO_STAGE} run_id=${RUN_ID}"
...
accelerate launch \
  ...
  --num_machines "${NUM_MACHINES}" \
  "${MULTI_ARGS[@]}" \
  "${V2_ROOT}/scripts/train_v2.py" \
  "output_dir=${V2_ROOT}/runs/${TASK_BASENAME}/${RUN_ID}" \
  "resume=${BASE_CKPT}" \
  "model.flux2_src_path=${FLUX2_SRC}" \
  "model.flux2_model_path=${FLUX2_MODEL_PATH}" \
  "model.ae_model_path=${FLUX2_AE_MODEL_PATH}" \
  "model.qwen3_model_spec=${QWEN3_MODEL_SPEC}" \
