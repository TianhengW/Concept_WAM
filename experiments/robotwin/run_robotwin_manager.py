...
def _parse_eval_phases(raw_phases: Any) -> list[str]:
    if raw_phases is None:
        phases = ["clean", "random"]
    elif isinstance(raw_phases, str):
        text = raw_phases.strip()
        if text.startswith("[") and text.endswith("]"):
            text = text[1:-1]
        phases = [item.strip().strip("\"'") for item in text.split(",") if item.strip()]
    else:
        phases = [str(item).strip() for item in raw_phases if str(item).strip()]

    allowed = {"clean", "random"}
    invalid = [phase for phase in phases if phase not in allowed]
    if invalid:
        raise ValueError(
            f"Unsupported MULTIRUN.phases values: {invalid}. "
            "Expected values from ['clean', 'random']."
        )
    if not phases:
        raise ValueError("MULTIRUN.phases must contain at least one phase.")
...
def _parse_gpu_ids(raw_gpu_ids: Any, *, num_gpus: int) -> list[int]:
...
def main(cfg: DictConfig):
    ...
    gpu_ids = _parse_gpu_ids(cfg.MULTIRUN.get("gpu_ids", None), num_gpus=num_gpus)
    selected_phases = _parse_eval_phases(cfg.MULTIRUN.get("phases", None))
    ...
    if task_name_cfg is None or str(task_name_cfg).strip() == "":
        ...
        tasks = [t.strip() for t in str(task_name_cfg).split(",") if t.strip()]
    ...
    phase_to_next_phase = {
        phase: selected_phases[idx + 1] if idx + 1 < len(selected_phases) else None
    ...
    log(
        ...
        f"max_tasks_per_gpu={max_tasks_per_gpu} phases={selected_phases} output_dir={run_output_dir}"
    )
